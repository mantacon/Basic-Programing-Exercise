...
void quicksort(struct student dt[], int left, int right, int *change);
...
int main(int argc, char *argv[]){
    ...
    if((fp = fopen(argv[1], "r")) != NULL){
        ...
        printf("************************\n");
        printout(dt, n);
        printf("比較回数:%d 入れ替え回数:%d", (n) * (int)log2(n), change);
        fclose(fp);
    }else{
...
void quicksort(struct student a[], int left, int right, int *change){
    int pivot; // 基準値用変数
    int i, j;        // 配列内の位置
    struct student temp;
